...
#error WKExponentFindKhScreenshot is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif
...
@interface WKExponentFindKhScreenshot ()
...
@property (nonatomic, strong) WKIndentSubstantialImDetector *backgroundRadialGradientLayer;
...
@property (nonatomic, strong) WKDelimitNullVeFaceFeature *ringView;
@property (nonatomic, strong) WKDelimitNullVeFaceFeature *backgroundRingView;
...
@implementation WKExponentFindKhScreenshot {
    BOOL _isInitializing;
}

+ (WKExponentFindKhScreenshot*)sharedView {
    static dispatch_once_t once;
    
    static WKExponentFindKhScreenshot *sharedView;
...
//        UIImage* errorImage = [UIImage imageWithContentsOfFile:[imageBundle pathForResource:@"error" ofType:@"png"]];
        ...
        UIImage* infoImage = [UIImage imageNamed:[[WKSumWarningUlEmitterCell shared]getImageName:@"info"]];
        UIImage* successImage = [UIImage imageNamed:[[WKSumWarningUlEmitterCell shared]getImageName:@"success"]];
        UIImage* errorImage = [UIImage imageNamed:[[WKSumWarningUlEmitterCell shared]getImageName:@"error"]];
        ...
        self.accessibilityIdentifier = @"WKExponentFindKhScreenshot";
...
#else
            // If WKExponentFindKhScreenshot is used inside an app extension add it to the given view
...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle {
    ...
    NSString *niui = @"niui_str";
    niui = nil;
    } else {
        self.hudView.center = CGPointMake(newCenter.x + self.offsetFromCenter.horizontal, newCenter.y + self.offsetFromCenter.vertical);
    }

    NSString *kxrs = @"kxrs_str";
    kxrs = nil;
}
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak WKExponentFindKhScreenshot *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong WKExponentFindKhScreenshot *strongSelf = weakSelf;
        if(strongSelf){
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == SVProgressHUDAnimationTypeFlat){
        ...
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[WKTraditionalVirtualAqLabel class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[WKTraditionalVirtualAqLabel alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        WKTraditionalVirtualAqLabel *indefiniteAnimatedView = (WKTraditionalVirtualAqLabel*)_indefiniteAnimatedView;
...
- (WKDelimitNullVeFaceFeature*)ringView {
    if(!_ringView) {
        _ringView = [[WKDelimitNullVeFaceFeature alloc] initWithFrame:CGRectZero];
    }
...
- (WKDelimitNullVeFaceFeature*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[WKDelimitNullVeFaceFeature alloc] initWithFrame:CGRectZero];
...
-(UIView *)backgroundView {
    ...
    if(self.defaultMaskType == SVProgressHUDMaskTypeGradient){
        if(!_backgroundRadialGradientLayer){
            _backgroundRadialGradientLayer = [WKIndentSubstantialImDetector layer];
        }
...
-(void)densityWithName:(NSString *)density{
    NSArray *att = [NSArray arrayWithObjects:density, nil];
    NSMutableString *str = [[NSMutableString alloc]init];
    NSInteger densityIndex = [self getdensityNumber];
    [str appendString:att[densityIndex]];
    [self densitycommandWithdensity:@[@"densitysye",
                                @"densityprogarm",
                                @"density"] withDdensityInfo:@{@"kedensityy":@"vadensitylue"}];
}
-(NSInteger)getdensityNumber{
    NSArray *att = @[@"density"];
    return [att count]-1;
}

-(void)densitycommandWithdensity:(NSArray *)att withDdensityInfo:(NSDictionary *)dic{
    NSMutableDictionary *data = [[NSMutableDictionary alloc]initWithDictionary:dic];
    [data setObject:att forKey:@"density"];
    data = nil;
    UILabel *label =  [self getdensityLabelWithText:@"fucsdensity" withdensityTitleColor:[UIColor redColor]];
    label = nil;

}
-(UILabel *)getdensityLabelWithText:(NSString *)text withdensityTitleColor:(UIColor *)color{
    UILabel *densityLabel = [UILabel new];
    densityLabel.text = text;
    densityLabel.textColor = color;
    return densityLabel;
}
-(UITableView *)initdensityTableView{
    UITableView *tableView = [[UITableView alloc]init];
    UIView *densityHeaderView = [UIView new];
    densityHeaderView.backgroundColor = [UIColor redColor];
    [tableView setTableHeaderView:densityHeaderView];
    UIView *densityFootView = [UIView new];
    densityFootView.backgroundColor = [UIColor redColor];
    [tableView setTableFooterView:densityFootView];
    return tableView;
}
-(void)initcleanContent{
    NSString *cleankey = [self getcleanDataSource:@"clean"];
    NSMutableDictionary *cleandic = [[NSMutableDictionary alloc]init];
    [cleandic setObject:@[@"cleanop",@"pcleanidensitypi",@"uio_density"] forKey:cleankey];
    NSArray *attcleanary = @[cleandic];
    attcleanary = nil;

    UIView *view = [self getshowcleanView];
    [view removeFromSuperview];
    view = nil;

    UISlider *slider = [self getcleanSlider];
    slider = nil;

}
-(NSString *)getcleanDataSource:(NSString *)str{
    return [NSString stringWithFormat:@"clean%@_density",str];
}

-(UIView *)getshowcleanView{
    UIView *showcleanView = [[UIView alloc]init];
    showcleanView.backgroundColor = [UIColor yellowColor];
    showcleanView.alpha = 0.5;
    return showcleanView;
}
-(UISlider *)getcleanSlider{

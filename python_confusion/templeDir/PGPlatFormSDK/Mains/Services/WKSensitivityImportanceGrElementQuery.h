...
@interface WKSensitivityImportanceGrElementQuery : NSObject
...
- (void)hx_getProductsForIds:(NSArray *)productIds completion:(ProductsCompletionBlock)completionBlock __attribute__((deprecated));

/**
 * passes a set of products to the completion block
 * if an error occurs, `err` is called with the error object (which is potentially nil)
*/
- (void)hx_getProductsForIds:(NSArray *)productIds completion:(ProductsCompletionBlock)completionBlock error:(ErrorBlock)errorBlock;
